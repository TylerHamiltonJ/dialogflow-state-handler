...
class WebhookClient {
   ...
   * @param {boolean|null} endConversation (boolean) if the conversation should end
   */
  reply(response, state, endConversation) {
    if (!!endConversation) {
      this.endConversation_ = true;
    }
  ...
  handleRequest(handler) {
    const contexts = this.contexts.find((f) => f.name === "session");
    this.data = contexts && contexts.parameters || {};
   ...
   */
  send_() {
    this.context.set("session", 99, { ...this.data });
